...
class BaseDataset(Dataset):
    ...
    def full_init(self):
        ...
        if self.pse != None:
            if self.pse.split('_')[0] == 'v2':
                self.psedata_v2process()
    ...
    def psedata_v2process(self):
        percentage = 0.4
        psedata_path = self.data_root + 'label_list/annotation_list_' + str(self.pse.split('_')[1])
        fp = open(psedata_path, 'r')
        label_list = json.load(fp)
        kpts_data = np.load(self.pse_kpt, allow_pickle=True)
        kpt_data = kpts_data.item()

        kpt_confident = {}
        for k_id, k_val in kpt_data.items():
            kpt_confident[k_id] = []
            for k_b in range(k_val.shape[0]):
                confi = []
                for k_n in range(k_val.shape[1]):
                    confi.append(k_val[k_b][k_n][2])
                kpt_confident[k_id].append(confi)
            kpt_confident[k_id] = np.array(kpt_confident[k_id])

        sorted_confidence = np.zeros(1)
        for k in kpt_data:
            for k1 in range(kpt_data[k].shape[0]):
                sorted_confidence = np.concatenate((sorted_confidence, kpt_data[k][k1, :, 2]), axis=0)
        sorted_confidence = np.sort(sorted_confidence)
        thre = sorted_confidence[int(percentage * sorted_confidence.shape[0])]
        for k in kpt_data:  # 如果大于这个阈值的置信度，那么就输出1否则0
            for k1 in range(kpt_data[k].shape[0]):
                kpt_data[k][k1, :, 2] = (kpt_data[k][k1, :, 2] > thre).astype(np.float32)
        real_num = 0
        pse_num = 0
        for index, data in enumerate(self.data_list):
            img_name = data['img_path'].split('/')[-1]
            img_id = int(img_name.split('.')[0])
            if img_id not in label_list:
                pse_num = pse_num + 1
                kpts = kpt_data[img_id]
                kpts_con = kpt_confident[img_id]
                kpt = []
                kpt_vis = []
                for nu in range(0, kpts.shape[0]):
                    id = int(kpts[nu][0][3])
                    if id == data['id']:
                        kpt = kpts[nu, :, :2]
                        kpt_vis = kpts[nu, :, 2]
                        kpt_conf = kpts_con[nu]
                        break

                if self.data_list[index]['keypoints'].shape[0] > 1:
                    print('mistake!')
                self.data_list[index]['keypoints'][0] = kpt
                self.data_list[index]['keypoints_visible'][0] = kpt_vis
                self.data_list[index]['use_label'] = np.array([1])
                self.data_list[index]['keypoints_confident'] = np.array([kpt_conf])
            else:
                real_num = real_num + 1
                self.data_list[index]['use_label'] = np.array([0])

        print('Psedata Loading finish!')
