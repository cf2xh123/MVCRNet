...
class ImgDataPreprocessor(BaseDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> Union[dict, list]:
        ...
        if 'img_w' in data['data_samples'][0]:
            data['data_samples'] = self.imgw_precess(data['data_samples'])
    ...
    def imgw_precess(self,data_sample):
        batch_inputs = []
        for nu in range(len(data_sample)):
            img_w = data_sample[nu].img_w
            if self._channel_conversion:
                img_w = img_w[[2, 1, 0], ...]
                # Convert to float after channel conversion to ensure
                # efficiency
                ...
                if self._enable_normalize:
                    if self.mean.shape[0] == 3:
                        assert img_w.dim(
                        ) == 3 and img_w.shape[0] == 3, (
                            'If the mean has 3 values, the input tensor '
                            'should in shape of (3, H, W), but got the tensor '
                            f'with shape {img_w.shape}')
                    img_w = (img_w - self.mean.cpu()) / self.std.cpu()
                batch_inputs.append(img_w)
        batch_inputs = stack_batch(batch_inputs, self.pad_size_divisor,
                                       self.pad_value)
        for n in  range(len(batch_inputs)):
                data_sample[n].set_metainfo({'img_w':batch_inputs[n]})
        return  data_sample
