...
@MODELS.register_module()
class SpatialTemporalSmoothingEMA(BaseAveragedModel):
    ...
    def __init__(self,
        ...
        if momentum > 0.5:
            print_log(
                ...
                level=logging.WARNING)
        self.momentum = momentum
        self.perserving_rate = perserving_rate #空间阈值

    def avg_func(self, averaged_param: Tensor, source_param: Tensor,
                 iter: int,iter_max) -> None:
        """Compute the moving average of the parameters using exponential
        moving average.
        ...
        if tmp_prob <self.perserving_rate:
            ...
            m = self.get_param_momentum(iter,iter_max)
            averaged_param.lerp_(source_param, m)

    def get_param_momentum(self,iter,maxiter):
        return 1.0 - (1.0 - self.momentum) * (
            (math.cos(math.pi * iter / maxiter) + 1) * 0.5
        )
    def update_parameters(self, model: nn.Module,iter : int,iter_max:int) -> None:
        """Update the parameters of the model. This method will execute the
        ``avg_func`` to compute the new parameters and update the model's
        parameters.
    ...
    def update_parameters111(self, student_encoder: nn.Module,iter:int ,max_iter:int) -> None:
        """Update the parameters of the model. This method will execute the
        ``avg_func`` to compute the new parameters and update the model's
        parameters.
        ...
        Args:
            model (nn.Module): The model whose parameters will be averaged.
        """
        m = self.get_param_momentum(iter,max_iter)
        for p1, p2 in zip(student_encoder.parameters(), self.module.parameters()):
        ...
        if not self.update_buffers:
            ...
            # If not update the buffers,
            # keep the buffers in sync with the source model.
            for b_avg, b_src in zip(self.module.buffers(), student_encoder.buffers()):
                b_avg.data.copy_(b_src.data.to(b_avg.device))
        self.steps += 1
