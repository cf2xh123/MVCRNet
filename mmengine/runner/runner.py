...
from mmengine.model import (MMDistributedDataParallel, convert_sync_batchnorm,
                            is_model_wrapper, revert_sync_batchnorm, ExponentialMovingAverage,
                            SpatialTemporalSmoothingEMA)
...
class Runner:
    ...
    def __init__(
        ...
        self.ema_model = None
        self.re_model = copy.deepcopy(self.model).eval()
        # wrap model
    ...
    def train(self) -> nn.Module:
        ...
        if self.train_dataloader.dataset.pse_vision().split('_')[0] == 'v2':
            self.ema_model = ExponentialMovingAverage(self.model, update_buffers=True, device='cuda', momentum=0.0002)
            #self.ema_model = SpatialTemporalSmoothingEMA(self.model,update_buffers= True,device = 'cuda',momentum=0.01,perserving_rate=0.7)
            for param in self.ema_model.parameters():
                param.detach()
            print('Finish to build ema model')
            self.pes = 'v2'
