...
class EpochBasedTrainLoop(BaseLoop):
    ...
    def run_iter(self, idx, data_batch: Sequence[dict]) -> None:
        ...
        if self.runner.re_model != None:
            self.runner.re_model =copy.deepcopy(self.runner.model).eval()
        outputs = self.runner.model.train_step(
            data_batch, optim_wrapper=self.runner.optim_wrapper,epoch = self.epoch,ema_model = self.runner.ema_model,re_model = self.runner.re_model)
        if self.runner.ema_model != None:
            self.runner.ema_model.update_parameters(self.runner.model)#, self._iter, self.max_iters)
        self.runner.call_hook(
            'after_train_iter',
            batch_idx=idx,
