...
class CocoMetric_Cls(BaseMetric):
    ...
    def __init__(self,
        ...
        self.coco.anno_file = 'ffffff'
        self.anns_cls(cls_val)
    ...
    def anns_cls(self,cls_val):
        ...
        cats = self.coco.cats
        for ann_id in anns:
            cls = anns[ann_id]['category_id']
            img_id = anns[ann_id]['image_id']
            if cls not in cls_val:
                del_anns.append(ann_id)
                if img_id not in del_img:
                    del_img.append(img_id)

        for id in del_anns:
            anns.pop(id)
        for id_im in del_img:
            imgs.pop(id_im)
