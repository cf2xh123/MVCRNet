...
class BaseCocoStyleDataset(BaseDataset):
    ...
    def parse_data_info(self, raw_data_info: dict) -> Optional[dict]:
        ...
        data_info = {
            ...
            'use_label' : [-1],
            'keypoints_confident' : np.zeros([1,17])
        }
    ...
    def filter_data(self) -> List[dict]:
        ...
        if self.pse!=None:
            if self.pse.split('_')[0] == 'v1':
                data_list = self.PsedataProcess(data_list)
                dataid = []
                for data in data_list:
                    if data['img_id'] not in dataid:
                        dataid.append(data['img_id'])
                print(dataid)
            elif self.pse.split('_')[0] == 'v3':
                data_list = self.PsedataProcess_v3(data_list)
    ...
    def PsedataProcess(self,data_list):
        import json
        psedata_path = self.data_root + 'label_list/annotation_list_' + str(self.pse.split('_')[1])
        fp = open(psedata_path, 'r')
        imageid_annot = json.load(fp)

        newdata = []
        for index, data in enumerate(data_list):
            data_id = data['img_id']
            if data_id in imageid_annot:
                newdata.append(data)
        return newdata

    def PsedataProcess_v3(self,data_list):
        import json
        father_cls = []
        all_clss = self.metainfo['CLASSES']
        for cls in all_clss:
            if cls['supercategory'] not in father_cls:
                father_cls.append(cls['supercategory'])
        newdata = []
        for index, data in enumerate(data_list):
            data_cls_id = data['raw_ann_info']['category_id']
            #data_cls_father = all_clss[data_id]['supercategory']
            if data_cls_id  == 6:
                newdata.append(data)
        return newdata
