...
@TRANSFORMS.register_module()
class GetWeaklyimg(BaseTransform):
    def transform(self, results: dict) -> dict:
        img_ori = results['img'].copy()
        results['img_w'] = img_ori
        return results
...
class Albumentation(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        #cv2.imshow(str(results['img_id']),  results['img'])
        results_albu = {}
        ...
        cv2.imshow(str(results['img_id'])+'_trange',  results['img'])
        cv2.waitKey(0)
        '''
        return results
