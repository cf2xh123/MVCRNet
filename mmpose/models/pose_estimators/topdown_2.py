...
class TopdownPoseEstimator(BasePoseEstimator):
    ...
    def loss(self, inputs: Tensor, data_samples: SampleList,epoch:int =None,ema_model=None) -> dict:
        ...
        if self.with_head:
            ...
            pred_out = (pred_x,pred_y)
            out_ori = (pred_x.clone(),pred_y.clone())
        ...
        if ema_model != None:
            ...
            out_ema = ema_model(inputs_w.clone(), data_samples)
            ...
            loss_all = {#'loss_kpt1' : loss_sm['loss_kpt']  ,
        ...
        if epoch >= re_epoch:
          with torch.no_grad():
            ...
            input_trans_res = self._forward(input_trans)
            ...
            img_re,r_re = self.tensor_rat(img_re.clone(),80,0.5)
            out_re = self._forward(img_re.clone())
            ...
            out_ema_re = ema_model(input_ema, data_samples)
            ...
            for bat_size in range(batch_shape):
                ...
                for kpt_size in range(kpt_shape):
                    if loss_weight_re[bat_size][kpt_size] == 5:
                        print(12345)
                        ...
                        if kpt_re[bat_size]['keypoint_scores'][0][kpt_size] > 0.9 and \
                                kpt_trans[bat_size]['keypoint_scores'][0][kpt_size] > 0.9 and kpt_ema_re[bat_size]['keypoint_scores'][0][kpt_size] > 0.9:
                            new_x = (x1 + x2 + x3) / 3
                            new_y = (y1 + y2 + y3) / 3
                            '''
                            img = inputs_w[bat_nu].cpu().numpy().transpose(1, 2, 0)
                        ...
                        elif d < 1.6 and  kpt_re[bat_size]['keypoint_scores'][0][kpt_size] >= 0.4 and \
