    ...
    def loss(self, inputs: Tensor, data_samples: SampleList,epoch:int =None,ema_model=None,re_model=None) -> dict:
        ...
        if epoch >= re_epoch:
          with torch.no_grad():
            ...
            keypoint_weights_ori = torch.cat(
                [
            ...
            keypoint_weights_ori[keypoint_weights_ori<0.4] = 0
                ...
                for kpt_size in range(kpt_shape):
                    if loss_weight_re[bat_size][kpt_size] == 1:
                        ...
                        if kpt_re[bat_size]['keypoint_scores'][0][kpt_size] > 0.8 and \
                                kpt_trans[bat_size]['keypoint_scores'][0][kpt_size] >0.8 and kpt_ema_re[bat_size]['keypoint_scores'][0][kpt_size] >0.8:
                            ...
                            kpt_gt[bat_size]['keypoints'][0][kpt_size][1] = new_x, new_y
                            kpt_re_w[bat_size][kpt_size] = 1
                    #elif data_samples[bat_size].gt_instances['use_label'][0]==1:
                        ...
                        #a = keypoint_weights_ori[bat_size][kpt_size]



                        if min_conf >= conf_ori and conf_ori >= 9999:
                            x1 = kpt_trans[bat_size]['keypoints'][0][kpt_size][0]
                            y1 = kpt_trans[bat_size]['keypoints'][0][kpt_size][1]
                            x2 = kpt_re[bat_size]['keypoints'][0][kpt_size][0]
                            y2 = kpt_re[bat_size]['keypoints'][0][kpt_size][1]
                            ...
                            if d <= 6.4:
                              ...
                              kpt_gt[bat_size]['keypoints'][0][kpt_size][0], \
                              kpt_gt[bat_size]['keypoints'][0][kpt_size][1] = new_x, new_y
                              keypoint_weights_ori[bat_size][kpt_size] = torch.tensor((conf_re+conf_trans+conf_ema_re)/3)
                            elif  conf_ema_re>conf_re and conf_ema_re>conf_trans:
                              kpt_gt[bat_size]['keypoints'][0][kpt_size][0], \
                              kpt_gt[bat_size]['keypoints'][0][kpt_size][1] = x3, y3
                              keypoint_weights_ori[bat_size][kpt_size] = torch.tensor(conf_ema_re)
                            elif conf_re > conf_ema_re and conf_re > conf_trans:
                              kpt_gt[bat_size]['keypoints'][0][kpt_size][0], \
                              kpt_gt[bat_size]['keypoints'][0][kpt_size][1] = x2, y2
                              keypoint_weights_ori[bat_size][kpt_size] = torch.tensor(conf_re)
                            elif conf_trans > conf_re and conf_trans > conf_re:
                              kpt_gt[bat_size]['keypoints'][0][kpt_size][0], \
                              kpt_gt[bat_size]['keypoints'][0][kpt_size][1] = x1, y1
                              keypoint_weights_ori[bat_size][kpt_size] = torch.tensor(conf_trans)
          ...
          _, avg_acc, _ = simcc_pck_accuracy(
              output=to_numpy(re_out),
              target=to_numpy(gt_simcc),
              simcc_split_ratio=self.head.simcc_split_ratio,
              mask=to_numpy(keypoint_weights_ori) > 0,
          )
