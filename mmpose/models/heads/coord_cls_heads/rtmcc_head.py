...
class RTMCCHead(BaseHead):
    ...
    def loss(
        ...
        if batch_data_samples[0].gt_instances.use_label[0] !=-1:
            use_labels = torch.cat([
                torch.from_numpy(d.gt_instances.use_label)
                for d in batch_data_samples
            ])



        pred_simcc = (pred_x.clone(), pred_y.clone())
        gt_simcc = (gt_x.clone(), gt_y.clone())

        # calculate losses
        losses = dict()
        if batch_data_samples[0].gt_instances.use_label[0] !=-1:
            if self.loss_type == 'Pre_loss':
             #loss ,target_out= self.loss_module(pred_simcc, gt_simcc, keypoint_weights,use_labels,train_cfg)
             loss ,loss_weight = self.loss_module(pred_simcc, gt_simcc, keypoint_weights, use_labels, train_cfg)
            else:
        ...
        if self.loss_type == 'Pre_loss':
            return losses,pred_x, pred_y,keypoint_weights,loss_weight,(gt_x.clone(), gt_y.clone())#,target_out
        else:
            return losses, pred_x, pred_y, keypoint_weights,0,0  # ,target_out
